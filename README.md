...
| Fast Forward | Cycle trick play: 2× → 5× → 10× → 30× → normal |
| Rewind | Cycle trick play: −2× → −5× → −10× → −30× → −2× |
| Play/Pause | Toggle playback |
| Stop | Stop and return to browse |
| Back | Hide controls overlay / exit player |

Trick play works by pausing the player and seeking by a fixed number of seconds per interval (333 ms), producing visually distinct speeds in both directions without using ExoPlayer's `setPlaybackSpeed()`.
